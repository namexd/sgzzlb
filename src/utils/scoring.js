import { findGeneralById, findTacticById, getAllTactics } from "./catalog";

const TROOP_KEY = { 骑兵: "cavalry", 盾兵: "shield", 弓兵: "bow", 枪兵: "spear", 器械: "siege" };
const APTITUDE_SCORE = { S: 100, A: 82, B: 64, C: 42, "": 55 };

const SCENARIO_BONUS = {
  pk: { name: "PK赛季", tags: ["控", "辅", "谋", "战"], text: "PK赛季按高强度环境处理，控制、减伤、治疗和稳定输出权重更高。" },
  pioneer: { name: "开荒", tags: ["医", "辅", "盾"], text: "开荒按低损和稳定性处理，治疗、减伤和兵种适性权重更高。" },
  war: { name: "打架", tags: ["控", "武", "谋"], text: "打架按对抗环境处理，控制、爆发和速度压制权重更高。" }
};
...
function getGeneral(generalId) {
  return typeof generalId === "object" ? generalId : findGeneralById(generalId);
}

function getTactic(tacticId) {
  return typeof tacticId === "object" ? tacticId : findTacticById(tacticId);
}

function getAptitude(general, troop) {
  const key = TROOP_KEY[troop] || "cavalry";
  const rank = general && general.arms ? general.arms[key] || "" : "";
  return { rank, score: APTITUDE_SCORE[rank] || APTITUDE_SCORE[""] };
}

export function isTacticCompatible(tactic, troop) {
...
function buildDimension(label, score, reason) {
  return { label, score: Math.round(clamp(score, 0, 100)), reason };
}
...
export function analyzeLineup(input) {
  ...
  if (generals.length < 3) validation.push("请至少选择 3 名武将，评分才有参考价值。");
  if (new Set(generals.map((item) => item.id)).size !== generals.length) validation.push("同一武将不能重复上阵。");

  const totalCost = generals.reduce((sum, item) => sum + (item.cost || 0), 0);
  if (totalCost > 20) validation.push(`当前统御为 ${totalCost}，已超过常规 20 御上限。`);

  const statBase = average(generals.flatMap((item) => [item.stats.force, item.stats.intellect, item.stats.command]));
  ...
  const roleCount = tacticRoles.reduce(
    (acc, item) => { Object.keys(acc).forEach((key) => { if (item[key]) acc[key] += 1; }); return acc; },
    { heal: 0, control: 0, damage: 0, defense: 0, speed: 0 }
  );
  const roleCoverage = Math.min(roleCount.damage, 3) * 14 + Math.min(roleCount.control, 2) * 12 + Math.min(roleCount.defense, 2) * 10 + Math.min(roleCount.heal, 1) * 12 + Math.min(roleCount.speed, 1) * 6;
  ...
  const dimensions = [
    buildDimension("属性基础", statScore, `三名武将核心属性均值约 ${Math.round(statBase)}，红度加成按预览权重计入。`),
    buildDimension("兵种适性", aptitudeScore, `${troop}适性为 ${aptitudes.map((item) => item.rank || "-").join(" / ")}。`),
    buildDimension("战法协同", tacticScore, `当前战法覆盖输出 ${roleCount.damage}、控制 ${roleCount.control}、减伤 ${roleCount.defense}、治疗 ${roleCount.heal}。`),
    buildDimension("阵营标签", identityScore, `同阵营 ${sameFactionCount} 人，命中当前场景标签 ${scenarioHit} 个。`),
  ...
  const explanations = [dimensions[0].reason, dimensions[1].reason, dimensions[2].reason, scenarioInfo.text];

  const weaknesses = [];
  if (weakAptitudes.length) weaknesses.push(`${weakAptitudes.map((item) => `${item.general.name}${item.aptitude.rank || "-"}`).join("、")} 的${troop}适性偏低。`);
  if (incompatibleTactics.length) weaknesses.push(`${incompatibleTactics.map((item) => item.name).join("、")} 与当前兵种限制不匹配。`);
  ...
  const confidence = validation.length ? "低" : tactics.length < 6 ? "中" : "中高";

  return { totalScore, totalCost, troop, scenario, scenarioName: scenarioInfo.name, confidence, validation, dimensions, explanations, weaknesses, replacements, premiumHints: ["高级订阅可展开完整替代战法池。", "高级订阅可批量预览主流环境队伍对位。"] };
}

export function compareLineups(ownReport, enemyReport) {
...
export default { analyzeLineup, compareLineups, getAptitude, isTacticCompatible, classifyTacticText };
