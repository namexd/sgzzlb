...
const GENERAL_SORT = { 魏: 1, 蜀: 2, 吴: 3, 群: 4 };
...
export function getTroopTactics() {
  return [...catalog.troopTactics].sort(byName);
}

export function findGeneralById(id) {
  return catalog.generals.find((item) => item.id === id) || null;
}

export function findTacticById(id) {
  return (
...
export function getTacticsByNames(names) {
